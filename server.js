...
app.patch('/api/admin/requests/:id',admin,async(req,res)=>{try{
  ...
  if(changes.length)await db('INSERT INTO request_history(request_id,action,details) VALUES($1,$2,$3)',[id,old.status!==status?'Atualização da solicitação':'Edição da solicitação',changes.join(' | ')]);
