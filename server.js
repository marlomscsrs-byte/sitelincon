...
async function init(){
 ...
 CREATE TABLE IF NOT EXISTS notifications(id SERIAL PRIMARY KEY,user_id INT REFERENCES users(id) ON DELETE CASCADE,type TEXT NOT NULL,title TEXT NOT NULL,message TEXT NOT NULL,request_id INT REFERENCES requests(id) ON DELETE CASCADE,created_at TIMESTAMPTZ DEFAULT NOW(),read_at TIMESTAMPTZ);
...
app.patch('/api/admin/requests/:id',admin,async(req,res)=>{try{
  ...
  const current=await db('SELECT * FROM requests WHERE id=$1',[id]);
  if(!current.rows[0])return res.status(404).json({error:'Solicitação não encontrada.'});
  const old=current.rows[0];
  const allowedStatus=['Em análise','Em andamento','Concluída','Cancelada'];
  const status=allowedStatus.includes(String(req.body.status||''))?String(req.body.status):old.status;
  ...
  const typeId=req.body.type_id===null||req.body.type_id===''?null:Number(req.body.type_id);
  const areaId=req.body.area_id===null||req.body.area_id===''?null:Number(req.body.area_id);
  if(typeId!==null&&!Number.isInteger(typeId))return res.status(400).json({error:'Tipo inválido.'});
  if(areaId!==null&&!Number.isInteger(areaId))return res.status(400).json({error:'Área inválida.'});
  const priority=String(req.body.priority||old.priority);
  const desiredDate=req.body.desired_date?String(req.body.desired_date).slice(0,10):null;
  const r=await db('UPDATE requests SET title=$1,description=$2,type_id=$3,area_id=$4,priority=$5,status=$6,desired_date=$7,updated_at=NOW() WHERE id=$8 RETURNING *',[title,description,typeId,areaId,priority,status,desiredDate,id]);
  const changes=[];
  if(old.status!==status)changes.push(`Status: ${old.status} → ${status}`);
  if(old.title!==title)changes.push('Título alterado');
  if(old.description!==description)changes.push('Descrição alterada');
  if(old.type_id!==typeId)changes.push('Tipo alterado');
  if(old.area_id!==areaId)changes.push('Área alterada');
  if(old.priority!==priority)changes.push(`Prioridade: ${old.priority} → ${priority}`);
  if(String(old.desired_date||'').slice(0,10)!==String(desiredDate||''))changes.push('Prazo alterado');
  const note=String(req.body.note||'').trim();
  if(note)changes.push(`Observação: ${note}`);
  if(changes.length){
    const action=old.status!==status?'Atualização da solicitação':'Edição da solicitação';
    const details=changes.join(' | ');
    await db('INSERT INTO request_history(request_id,action,details) VALUES($1,$2,$3)',[id,action,details]);
    const notificationTitle=old.status!==status?'Solicitação atualizada':'Alteração na sua solicitação';
    const notificationMessage=old.status!==status?`${r.rows[0].protocol}: status alterado para ${status}.`:`${r.rows[0].protocol}: sua solicitação foi alterada. ${details}`;
    await db('INSERT INTO notifications(user_id,type,title,message,request_id) VALUES($1,$2,$3,$4,$5)',[old.user_id,'request_update',notificationTitle,notificationMessage,id]);
  }
  res.json({request:r.rows[0],message:'Solicitação atualizada com sucesso.'});
}catch(e){console.error('PATCH /api/admin/requests/:id:',e);res.status(500).json({error:'Não foi possível salvar as alterações. Verifique os dados informados.'})}});
...
app.post('/api/auth/forgot',async(req,res)=>{try{const username=String(req.body.username||'').trim().toLowerCase();const email=String(req.body.email||'').trim().toLowerCase();if(!username||!email)return res.status(400).json({error:'Informe o nome de usuário e o e-mail cadastrado.'});const r=await db('SELECT id,name,email,username FROM users WHERE LOWER(username)=LOWER($1) AND LOWER(email)=LOWER($2) LIMIT 1',[username,email]);if(!r.rows[0])return res.status(400).json({error:'Nome de usuário e e-mail não conferem com o cadastro.'});const raw=require('crypto').randomBytes(32).toString('hex');await db('DELETE FROM password_resets WHERE user_id=$1',[r.rows[0].id]);await db("INSERT INTO password_resets(user_id,token,expires_at) VALUES($1,$2,NOW()+INTERVAL '30 minutes')",[r.rows[0].id,raw]);res.json({ok:true,token:raw,message:'Dados conferidos. Agora defina uma nova senha.'})}catch(e){console.error('POST /api/auth/forgot:',e);res.status(503).json({error:'Não foi possível iniciar a recuperação de senha.'})}});
